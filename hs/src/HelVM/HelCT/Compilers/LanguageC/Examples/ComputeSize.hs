...
module HelVM.HelCT.Compilers.LanguageC.Examples.ComputeSize where
...
main = do
    ...
    let (pat,args')   = ((head . fromList) &&& (tail . fromList)) args
...
defineComp ty = CDeclExt (CDecl (map CTypeSpec (exportCompType $ derefTypeDefs ty)) [] undefNode)
    where
    derefTypeDefs = everywhere (mkT derefTypeDef `extT` replaceEnum)
...
genSizeTest typeDefs tys =
      either (\e -> error $ toText $ "Failed to parse " <> test <> ": " <> show e)
             fromExtDecl
