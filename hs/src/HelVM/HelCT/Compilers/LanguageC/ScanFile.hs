...
module HelVM.HelCT.Compilers.LanguageC.ScanFile where
...
main = do
    ...
    let (pat,opts,input_file) = case hasExtension (last $ fromList args) of
                              ...
                              False -> let (pat',args') = (last $ fromList args, init $ fromList args)
                                       in (Just pat',init $ fromList args',last $ fromList args')
